...
By considering the previous pointer example, we now have a rough idea how the VM might execute instructions.
...
After the call instruction in line 7, the VM starts executing the instructions of the \qVerb{rec} function.
...
Next, the instruction \qVerb{svari} saves the value of the parameter which was previously pushed onto the stack at the relative address 0.
...
In this case, the instruction compares if the current value of \qVerb{n} is equal to 0.
In line 16, the \qVerb{jmpfalse} instruction is executed.
This instruction jumps to the specified instruction index if the value on top of the stack is \qVerb{false}.
In this case, if the value on the stack is false, the parameter \qVerb{n} was not equal to 0.
...
In this case, the instruction subtracts 1 from the value of \qVerb{n} and pushes the result onto the stack.
Next, the \qVerb{rec} calls itself recursively using a previously explained \qVerb{call} instruction.
...
Next, the function decrements the memory pointer in order to deallocate used memory using the \qVerb{setmp} instruction in line 24.
...
Lastly, the \qVerb{ret} instruction is used to return from the function.
Now we have considered what happens if the value of \qVerb{n} was not equal to 0.

However, if the result of the comparison in line 15 is true, meaning that \qVerb{n} is equal to 0, the \qVerb{jmpfalse} instruction in line 16 does nothing.
In this case, the VM continues to the \qVerb{push} instruction in line 17.
...
The instruction at index 14 is \qVerb{setmp} in line 24.
