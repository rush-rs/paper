...
The \emph{saved} registers \qVerb{s1} — \qVerb{s11} are typically preserved across function calls.
...
\begin{itemize}
	...
	\item Transferring control to the procedure using the \qVerb{jal} or \qVerb{call} instruction.
	...
	\item Returning control back to the caller code using the \qVerb{ret} instruction since procedures can be called from multiple places in the program.
\end{itemize}
...
In the main-function, \qVerb{m} is incremented by 1 in line 4.
Next, in line 5, the \qVerb{foo} function is called using \qVerb{m} as its call argument.
...
Therefore, the \qVerb{foo} function only exits with the specified parameter \qVerb{n}.
...
\Lirsting[float=H, caption={Foo}, label={lst:riscv_simple}]{listings/riscv_simple.rush}
