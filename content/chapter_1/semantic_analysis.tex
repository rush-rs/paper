...
Because these statements are only valid inside loop bodies, the value of \qVerb{loop_count} must be $> 0$ when the analyzer encounters such a statement.
...
The Listing displays part of a rush program which uses a \qVerb{while} loop even though a \qVerb{loop} would suffice.
...
Most notably, the sub-expression \qVerb{1 + 42} has been transformed into \qVerb{42} during constant folding.
