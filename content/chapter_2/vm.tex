...
For instance, if the value of variable `\texttt{d}' in Figure~\ref{fig:rush_vm_linmem} was to be retrieved, the VM would need to access the storage cell with the index 4.
...
The mode is called \emph{relative} because all addresses are specified relative to \qVerb{mp}.
...
\begin{itemize}
    \item \TODO{Use e, not d for the examples}
    \item \TODO{what is MP pointing tom why can we take it as a base?}
\end{itemize}
