...
In line 6, the start of the `\enquote{entry}' block of the \qVerb{fib} function is declared using the block's name followed by a colon.
...
In line 7, the \qVerb{icmp slt}\footnote{Short for \enquote{integer compare (signed less than)}} is used in order to compare the runtime value of the parameter \qVerb{\%0} to a constant 2.
The boolean result is then saved in a virtual register named \qVerb{\%i\_lt}.
...
The instruction will only jump under the condition that the value of \qVerb{\%i_lt} is true.
...
Next, the `\texttt{add}' instruction in line 19 is used in order to calculate the sum of the return values of the recursive calls
...
\Lirsting[ranges={26-29,47-47}, caption={Parts of the Struct Definition of the rush LLVM \texttt{Compiler}}, label={lst:llvm_cmp_struct}, float=H]{deps/rush/crates/rush-compiler-llvm/src/compiler.rs}
...
% - Explain linking X
% - Difference `.o` vs binary X
% - LLVM's link-time optimizations  X
% - https://en.wikipedia.org/wiki/Linker_(computing)
% - Linker & loader book: P. 5 - 7 X
% - Insert figure from p. 7 X

\TODO{MORE ON LINKERS: \cite[p.~40]{Patterson2017}}
...
\begin{wrapfigure}{R}{0.5\textwidth}
	...
	\end{tikzpicture}
	\caption{How a Linker Works}\label{fig:linker}
\end{wrapfigure}~\cite[pp.~1-15]{Levine2000}
