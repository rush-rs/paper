...
\Lirsting[raw=true, wrap=o, fancyvrb={numbers=\OuterEdge}, wrap width={.33\textwidth}, caption={VM instructions matching the AST in~\ref{fig:tree_vs_vm_tree}.}, label={lst:tree_vs_vm_vm}]{listings/vm_faster_instructions.txt}
...
In the body of the loop, the current instruction is executed using the \qVerb{self.run_instruction} method.
...
Since the instruction pointer is incremented at the end of the method, the return-statement in line 193 is used to terminate this method early, preventing the increase of \qVerb{ip} at the end.
