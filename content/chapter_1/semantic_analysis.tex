...
In order to understand why type information is required at compile time,
...
The analyzer will first check if the program contains a \qVerb{main} function.
...
function body of the \qVerb{main} function. First, the analyzer examines the statements in the lines 2 and 3.
...
Here, the analyzer checks that the identifiers \qVerb{two} and \qVerb{three} refer to valid variables.
...
Because these statements are only valid inside loop bodies, the value of \qVerb{loop_count} must be $> 0$ at the point when the analyzer encounters such a statement.
...
If the vector contained any items, the \qVerb{main} function's declaration would include parameters.
If this was the case, the analyzer would generate an error message stating that the \qVerb{main} function must not take any parameters.
...
Secondly, in rush, the \qVerb{main} function's return type always has to be the unit type.
...
Therefore, the inner if-clause is only executed if the user has manually specified a result type of their \qVerb{main} function.
...
After the signature of the \qVerb{main} function has been validated, the method begins traversal of the function's body.
In line 490, the \qVerb{block} method of the analyzer is invoked using the function body as its first argument.
...
During the traversal of the \qVerb{main} function's body, the analyzer encounters two \qVerb{let} statements in line 2 and 3.
...
In line 144, the result type of a grouped expression is obtained by calling the \qVerb{result_type} method recursively.
...
Because the type of a non-existent variable is unknown, the placeholder \qVerb{Unknown} type would be used to prevent cascading errors.
...
\Lirsting[ranges={1807-1807,_1813-1822,1823-1823,_1828-1833}, caption={Validation of argument type compatibility in the analyzer.}, label={lst:analyzer_call_exit}, float=H]{deps/rush/crates/rush-analyzer/src/analyzer.rs}
...
\Lirsting[ranges={148-153}, caption={Method for determining whether an expression is constant.}, label={lst:expr_constant_impl}, float=H]{deps/rush/crates/rush-analyzer/src/ast.rs}

In order to make such optimization possible, each expression node in the analyzed AST has a method named \qVerb{constant}.
The method shown in Listing~\ref{lst:expr_constant_impl} is responsible for determining whether an expression is constant.
It returns \qVerb{true} if the expression is a constant integer, float, boolean or char.
...
\Lirsting[ranges={3-5}, caption={Redundant \qVerb{while} loop inside a rush program.}, wrap=L, wrap width=.25\textwidth, label={lst:redundant_while}]{listings/constant_true_while.rush}
...
The Listing~\ref{lst:redundant_while} displays part of a rush program which uses a \qVerb{while} loop even though a \qVerb{loop} would suffice.
A \qVerb{loop} implementation is more efficient since the condition check is omitted before each iteration.
...
\Lirsting[ranges={771-771,851-865,866-866}, caption={Loop transformation in the analyzer.}, label={lst:analyzer_loops}, float=H]{deps/rush/crates/rush-analyzer/src/analyzer.rs}
...
Most notably, the sub-expression \qVerb{1 + 42} has been transformed into \qVerb{43} during constant folding.
