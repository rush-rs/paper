...
The remaining \qVerb{an}-registers (\qVerb{a2}--\qVerb{a7}) should be primarely used as function call arguments.
...
However, the other \qVerb{fan}-registers (\qVerb{fa2}--\qVerb{fa7}) are meant to be used as float function arguments.
Just like the \qVerb{sn} registers, the \qVerb{fs0}--\qVerb{fs11} registers are usually preserved across function calls.
...
\begin{wrapfigure}{O}{0.39\textwidth}
	...
	\begin{tikzpicture}[xscale=0.9, yscale=0.7]
		...
		\cell{$n$\textsuperscript{th} stack argument} \cellcom{\texttt{8$n$(fp)}}
		\cell[padding]{...}
...
It is used for storing variables, following the alignment rules that were explained in Subsection~\ref{sec:alignment}.
...
If stack arguments were present, the would be placed upwards from \qVerb{fp}.
Each stack argument \emph{always} uses eight bytes of memory, regardless of the actual size of its content.
Thus, the $n$\textsuperscript{th} stack argument would be placed at the position \texttt{$8n$(fp)}.
...
In the case of \riscv{}, the calling convention consists of three main steps and is specified in a separate document~\cite{RiscvABI2022}.
...
Char-arithmetic requires external functionality since the bounds of the ASCII range should be regarded while performing operations.
For instance, in rush, calculating \qVerb{a+a} results in `66', even though `194' ($97 + 97$) would also be logical.
If the result is larger than `127`, `128` is subtracted from the result, if the result is less than `0', `128' is added to it.
