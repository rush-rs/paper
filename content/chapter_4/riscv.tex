...
\qVerb{fp} points to the address above the current frame.
The available space for functions ranges from \qVerb{-24(fp)} to \qVerb{0(sp)}.
...
\Lirsting[wrap=r, fancyvrb={numbers=right}, wrap width=0.41\textwidth, caption={Compiler output of the rush program in Listing~\ref{lst:riscv_simple2}.}, label={lst:riscv_simple_asm}, ranges={1-30, -4 51-53}]{listings/riscv_simple.s}
...
Next, the \qVerb{ld} (\textbf{l}oad \textbf{d}word\footnote{Here, \enquote{dword} indicates that 64 bits of data shall be loaded.}) instruction in line~19 is used in order to load the value of the global variable \qVerb{m} into the register \qVerb{a1}~\cite[Reference Card]{Patterson2017}.
...
Next, in line~21, the \qVerb{sd} (\textbf{s}tore \textbf{d}word) instruction saves the value of the register \qVerb{a2} at the memory location of the global variable \qVerb{m},
...
\begin{minipage}{.45\textwidth}
	\centering
	\Lirsting[float=H, ranges={10-10,18-24}, caption={Compiler output of the rush program in Listing~\ref{lst:rush_simple_pointer}.}, label={lst:rush_simple_pointer_asm}]{listings/generated/riscv_rush_simple_pointer.s}
	\vspace{.1cm}
\end{minipage}
...
\Lirsting[ranges={11-11,15-15,19-34}, caption={The \qVerb{Compiler} struct definition of the \riscv{} compiler.}, label={lst:riscv_compiler_attr}, float=h]{deps/rush/crates/rush-compiler-risc-v/src/compiler.rs}

\Lirsting[ranges={57-57,69-71,110-110}, caption={\riscv{}: The \qVerb{Instruction} enumeration.}, label={lst:riscv_instruction_enum}, float=h]{deps/rush/crates/rush-compiler-risc-v/src/instruction.rs}
...
\Lirsting[ranges={98-98,130-132,136-136}, caption={\riscv{}: The \qVerb{IntRegister} enumeration.}, label={lst:riscv_intregister_enum}, float=h]{deps/rush/crates/rush-compiler-risc-v/src/register.rs}
...
Next, the \qVerb{data_section} and \qVerb{rodata_section} fields are declared in lines~20--23.
...
\Lirsting[ranges={175-186}, float=h, label={lst:riscv_get_int_reg}, caption={\riscv{}: The \qVerb{get_int_reg} method.}]{deps/rush/crates/rush-compiler-risc-v/src/utils.rs}
...
For each register in that pool, the if-expression in lines~177--180 checks whether the current register \qVerb{reg} is contained in the \qVerb{used_registers} vector.
If this is not the case, the current register is still unused and can therefore be returned for usage.
Since the \qVerb{return} statement causes the function to return immediately,
...
For this, the \qVerb{use_reg} method, which only performs this one operation, is implemented.
When a register is no longer used, and can be overwritten again, it must be removed from the \qVerb{used_registers} vector.
For this, another simple method, called \qVerb{release_reg}, is implemented.
...
Only the first section of the pool, which contains the `\Verb{a}$n$' registers, can be seen.
...
\Lirsting[float=h, ranges={590-596,656-657}, caption={\riscv{}: Parts of the \qVerb{expression} method.}, label={lst:riscv_expression}]{deps/rush/crates/rush-compiler-risc-v/src/compiler.rs}
...
\Lirsting[float=h, ranges={755-755,791-791,841-858}, caption={\riscv{}: Parts of the \qVerb{infix_expr} method.}, label={lst:riscv_infix_expr}]{deps/rush/crates/rush-compiler-risc-v/src/compiler.rs}
...
Listing~\ref{lst:riscv_infix_expr} shows a part of the \qVerb{infix_expr} method of the rush compiler.
It consumes an annotated tree-node representing an analyzed infix-expression.
In line~845, the left-hand side expression is compiled, and its result register is saved in the variable \qVerb{lhs_reg}.
Next, in line~846, the previously discussed \qVerb{use_reg} method is used in order to mark the register specified by \qVerb{lhs_reg} as used.
...
In line~848, the right-hand side expression is compiled and its result register is saved in the variable \qVerb{rhs_reg}.
After this, \qVerb{lhs_reg} is marked as unused again.
Finally, in line~853, the \qVerb{infix_helper} method is called in order to insert the instructions for the actual calculation.
...
\Lirsting[
    ...
    caption={\riscv{}: Parts of the \qVerb{infix_helper} method.},
...
is required during compilation of both infix-expressions and certain assign-expressions, \mbox{\qVerb{a += 1}} for instance.
...
Since the \qVerb{add} instruction demands it, the operand registers are asserted to be integer registers via the \qVerb{into} method.
...
Here, the \qVerb{fsub} (\textbf{f}loat \textbf{sub}tract) instruction is used for subtraction.
In this case, the variable \qVerb{dest_regf} is used as the first operand because the resulting value is a float,
and these \qVerb{into} calls assert the registers to be float registers.
...
Listing~\ref{lst:riscv_define_main_fn} shows the \qVerb{define_main_fn} method.
It is only responsible for compilation of the \qVerb{main} function.
...
Secondly, the special instructions for calling the \qVerb{main} and \qVerb{exit} functions are not required.

\Lirsting[
	float=h,
	ranges={123-156},
    caption={\riscv{}: The \qVerb{define_main_fn} method.},
	label={lst:riscv_define_main_fn}
]{deps/rush/crates/rush-compiler-risc-v/src/compiler.rs}

The method takes a block, that is, a list of statements as its input.
In lines~124--126, the \qVerb{_start} label is created and exported so that the linker will later be able to refer to it.
Next, in lines~128--129, a new block with the \qVerb{main..main} label is created.
As explained previously, this block represents the start of the body of the \qVerb{main} function.
In line~132, a \qVerb{call} instruction, which is responsible for calling the \qVerb{main} function at the start of program execution, is inserted.
In lines~133--135, instructions for calling the \qVerb{exit} function using `0' as the exit code are inserted.
Next, in lines~137--139, a new epilogue label is generated, and the \qVerb{curr_fn} field is updated so that it contains this newly created epilogue label.
However, this label is not directly added to the \qVerb{blocks} vector since it should appear after the function body.
...
Then, the function body's contents are compiled by calling the \qVerb{function_body} method.
...
\Lirsting[
	float=h,
	ranges={66-87},
    caption={\riscv{}: The \qVerb{epilogue} method.},
	label={lst:riscv_epilogue}
]{deps/rush/crates/rush-compiler-risc-v/src/call.rs}

Listing~\ref{lst:riscv_epilogue} shows this \qVerb{epilogue} method.
...
The instruction insertion in lines~69--73 is responsible for an \qVerb{ld} instruction.
Here, the \qVerb{ld} instruction is used to restore the previously saved \qVerb{fp} register.
In lines~74--78, another \qVerb{ld} instruction for restoring the \qVerb{ra} register is inserted.
In lines~79--84, an \qVerb{addi} instruction is inserted in order to free the memory used for the function's stack frame.
...
It returns an \qVerb{Option<Pointer>}, where \qVerb{None} again represents the \qVerb{!} and \qVerb{()} types.
A \qVerb{Pointer} stores a relative memory address given a base register, \qVerb{fp} in this case.

\Lirsting[
	float=h,
	ranges={557-557,_561-567,580-587},
    caption={\riscv{}: The \qVerb{save_expr_on_stack} method.},
	label={lst:riscv_save_on_stack}
]{deps/rush/crates/rush-compiler-risc-v/src/compiler.rs}

In line~563, the expression is compiled, and the resulting register is saved in the variable \qVerb{reg}.
In line~565, the \qVerb{get_offset} method is used in order to calculate the frame pointer offset of a variable.
...
The \qVerb{match} block in line~567 executes different code based on the register type which the expression yielded.
In lines~580--583, the code responsible for float registers can be seen.
Here, the \qVerb{fsd} (\textbf{f}loat \textbf{s}tore \textbf{d}ouble) instruction is used in order to save the resulting register at the memory position relative to \qVerb{fp} which is specified in the variable \qVerb{offset}.
The code for integer registers is similar, but also differentiates between 8-bit and 64-bit data.
In line~586, the \qVerb{Pointer} abstraction representing the memory location of the expression is returned.
...
\Lirsting[
    ...
    start continued=true,
    caption={\riscv{}: The \qVerb{call_expr} method.},
...
The register spilling is performed by iterating over the currently used registers and calling the \qVerb{spill_reg} method for each register.
...
\Lirsting[
	...
	fancyvrb={firstline=2, lastline=31},
	continued=true,
]{deps/rush/crates/rush-compiler-risc-v/src/call.rs}

The next part of the \qVerb{call_expr} method is shown is Listing~\ref{lst:riscv_call_expr_2}.
...
For this, an \qVerb{fsd} instruction (or \qVerb{sd} for integers) is inserted in order to save the result of the current argument on the stack.
...
In line~131 of Listing~\ref{lst:riscv_call_expr_2}, the code calculating the offset relative to \qVerb{sp} can be seen.
Here, the value of \qVerb{spill_cnt} is simply multiplied by eight in order to obtain the correct offset, as each argument should use eight bytes.
In line~135, the \qVerb{spill_cnt} variable is incremented as described previously.
Lastly, the \qVerb{spill_param_size} variable is incremented by eight bytes, as floats require eight bytes of storage.
...
\Lirsting[
    ...
    caption={\riscv{}: The \qVerb{call_expr} method.},
	label={lst:riscv_call_expr_3}
]{deps/rush/crates/rush-compiler-risc-v/src/call.rs}

The final part of the \qVerb{call_expr} method is shown in Listing~\ref{lst:riscv_call_expr_3}.
The code in lines~165--173 is responsible for mangling the name of the called function so that it includes the \qVerb{main..} prefix.
...
The code in lines~179--185 is responsible for selecting an appropriate output register based on the function's return type, storing it in \qVerb{res_reg}.
...
This return register is required as an argument to the following \qVerb{restore_regs_after_call} method call.
...
In order to mitigate this issue, the method will alter the result register by inserting a \qVerb{mv} (\textbf{m}o\textbf{v}e) instruction if necessary.
Since the method returns the final output register which contains the call's result, it is used as the last expression of the block, thus also making it the return value of the \qVerb{call_expr} method.
...
In case it did not, a \qVerb{mv} instruction is used to move it there.
...
This is accomplished by the \qVerb{beqz} (\textbf{b}ranch if \textbf{eq}ual to \textbf{z}ero) instruction which jumps to the specified label if the value inside the first operand is zero~\cite[p.~105]{Waterman2019}.
...
This label starts a block with the code that follows the while-statement, here only the call to the \qVerb{exit} function.
...
In line~32, and unconditional jump, which jumps back to the \qVerb{while_head_0} label, follows.
Therefore, the loop begins its next iteration.
Using this approach, the compiler is able to generate instructions for the \qVerb{loop}, \qVerb{while}, and \qVerb{for} statements.
