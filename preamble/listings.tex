...
\newcommand{\TSInline}[2]{\ignorespaces\directlua{
    local escaped = string.gsub([[#2]], "'", [['"'"']])
    local handle, err = io.popen("ts2tex inline #1 '" .. escaped .. "'")
    if not handle then
        print(err)
        os.exit(1)
    end
...
%
    ...
    °\expandafter\VerbatimInput\expandafter[\args]°{|"ts2tex from-file #1 °\commandargs°"}
