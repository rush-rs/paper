...
\TSListing[ranges={10-16}, float=t, caption={The rush \texttt{Lexer} Struct Definition}, label={lst:lexer}]{deps/rush/crates/rush-parser/src/lexer.rs}
...
\begin{table}[h]
	...
	\begin{tabularx}{0.95\textwidth}{Ll}
		...
		\hline
		\TSInline{ebnf}{A = B , C ;}                 & \TSInline{rs}{struct A { b: B, c: C }}                                    \\
		\TSInline{ebnf}{A = B , [ C ] ;}             & \TSInline{rs}{struct A { b: B, c: Option<C> }}                            \\
		\TSInline{ebnf}{A = B , { C } ;}             & \TSInline{rs}{struct A { b: B, c: Vec<C> }}                               \\
		\TSInline{ebnf}{A = B , { C }- ;}            & \TSInline{rs}{struct A { b: B, c: Vec<C> }}                               \\
		\TSInline{ebnf}{A = B | C ;}                 & \TSInline{rs}{enum A { B(B), C(C) }}                                      \\
		\TSInline{ebnf}{A = B , ( '+' | '-' ) , C ;} & \gape{\makecell[l]{\TSInline{rs}{struct A { b: B, op: Op, c: C }}         \\\TSInline{rs}{enum Op { Plus, Minus }}}} \\
		\TSInline{ebnf}{A = B , [ ( X | Y ) , C ] ;} & \gape{\makecell[l]{\TSInline{rs}{struct A { b: B, c: Option<(XOrY, C)> }} \\\TSInline{rs}{enum XOrY { X(X), Y(Y) }}}} \\
	\end{tabularx}
\end{table}
...
\TSListing[ranges={172-173, 194-201}, float=h, label={lst:rush_tok_prec}, caption={Token Precedences in rush}, gobble=4]{deps/rush/crates/rush-parser/src/token.rs}

The \Verb{expression} method on the \Verb{Parser} struct is then modified to take a parameter for the current precedence.
\ldots

\TODO{figure for pratt parsing showing input `\Verb{(1+2*3)/4**5}' with precedences}
