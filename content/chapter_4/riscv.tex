...
\TODO{is the purpose of this label clear?}
...
\TODO{also show code of prologue?}
...
\Lirsting[float=h, ranges={558-565}, path prefix={deps/rush}, caption={The \qVerb{let_statement} Method in the RISC-V rush compiler}, label={lst:riscv_let_stmt}]{deps/rush/crates/rush-compiler-risc-v/src/compiler.rs}
...
In line 562 of Listing~\ref{lst:riscv_simple_asm}, this method is called and the resulting relative memory address is saved in the variable \qVerb{ptr}.
...
The rust code in Listing~\ref{lst:riscv_save_on_stack} displays parts of the \qVerb{save_expr_on_stack} which was called in line 562 of the previous listing.

\Lirsting[float=h, ranges={567-567,_571-577,590-597}, path prefix={deps/rush}, caption={The \qVerb{save_expr_on_stack} Method in the RISC-V rush compiler}, label={lst:riscv_save_on_stack}]{deps/rush/crates/rush-compiler-risc-v/src/compiler.rs}
...
\TODO{what about the previous example with !}
Otherwise, the method returns the relative memory address which will contain the value of the saved expression at runtime.
In line 573, the passed expression is compiled and the resulting register is saved in the variable \qVerb{reg}.
In line 575, the \qVerb{get_offset} method is used in order to calculate the fp-offset of variable.
...
Since the \qVerb{get_offset} method is used everytime values need to be saved on the stack,
...
The \qVerb{match} block in line 577 executes different code based on the register type which the expression yielded.
...
In line 596, the \qVerb{Pointer} abstraction representing the memory locatoin of the expression is returned.
...
the runtime value of the \qVerb{int_cnt} variable is 1 whilst the \qVerb{spill_param_size} variable holds 0.
...
\TODO{in the original section about calling convention, did I mention that every type uses 8 Bytes, no matter what?}
...
This way, values can be returned at any time whilst stopping execution of the function.
...
The instructions in the lines 28--31 are responsible for compiling the expression \qVerb{count += 1} in line 4 of the rush code.
