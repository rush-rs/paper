...
\begin{table}[h]
	...
	\begin{tabularx}{0.95\textwidth}{lLl}
        ...
        For & Like \qVerb{while}, also includes an update-expression which is executed after each iteration & \LirstInline{rush}{for i = 0; i < 10; i += 1 { /* */  }} \\
        ...
        Infix Expr & For performing mathematical and logical operations & \LirstInline{rush}{1 + n; 5 ** 2} \\
        Prefix Expr & Can also be used for mathematical and logical operations & \LirstInline{rush}{!false; -n} \\
        Variables & Associate a name with a value which can change during runtime & \LirstInline{rush}{let mut answer = 42} \\
        Cast Expr & Convert a value into another one with a different type & \LirstInline{rush}{42 as char} \\
	\end{tabularx}
\end{table}
