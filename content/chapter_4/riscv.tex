...
\begin{minipage}{.45\textwidth}
	\center
	\Lirsting[ranges={18-25},caption={Assembly Output of Rush Program in Listing~\ref{lst:asm_riscv_simple_sum}}, label={lst:rush_riscv_simple_sum}, float=H]{listings/generated/rush_simple_add.s}
\end{minipage}
...
Unlike the register allocation algorithm of a production ready compiler like LLVM, this one only tries to use the registers without causing conflicts like the previously explained one.
...
In order to choose an output register, each of those method uses the helper method \qVerb{alloc_ireg}.
This method returns the first available register from a register pool.
The register pool contains all possible registers which can hold a required data type.
This compiler contains two pools: one for integer registers and one for floating-point registers.

However, if 

\begin{itemize}
    \item If a register cannot be overwritten, mark it as \enquote{used}
    \item Each function decides which target register will hold its result
    \item Registers are acquired from a pool of registers
    \item the first free register from this pool is taken when a new register is requested
    \item For instance, a new integer register can be acquired by calling the \qVerb{self.alloc_ireg} method.
    \item If this function is called repeatedly, it would always return the same register, \qVerb{a0} for instance.
    \item Therefore, registers can be marked as \qVerb{used}.
    \item In this case, the method would return the next free register after the last used one
    \item \TODO{include graphic here}
    \item Such a pool exists for both integer and floating-point registers
    \item Include rust code which calls the \qVerb{alloc_ireg} and \qVerb{use_reg} methods (in the \qVerb{infix_expr} method)
