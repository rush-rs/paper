...
\TODO{For more general info:~\cite[p.~209]{Watson2017}}
...
However, since the VM is unable to traverse the AST, it depends on a compiler generating its input instructions.
...
\TODO{Figure which shows the cycle}
...
Since virtual machines are purely abstract constructs, meaning that they are implemented using software, design constraints are usually kept to a minimum.
...
From the point where the architecture is clear, implementing the VM should normally be a straight-forward task.
...
\begin{figure}[h]
	...
	\begin{minipage}{.3\textwidth}
		\Lirsting[raw=true, fancyvrb={frame=none}]{listings/vm_faster_instructions.txt}
	\end{minipage}
...
% TODO: use these subsubsections?
%\subsubsection{Struct Fields of the rush VM}

Listing~\ref{lst:vm_struct} displays the struct definition of the rush VM\@.
In line 18, a field called `\texttt{stack}' is declared.
...
Lastly, a field named `\texttt{call\_stack}' is declared.
This field also behaves like a stack and is responsible for managing function calls and returns.
More information on this field will be provided in later parts of this section.
...
% TODO: use these subsubsections?
% \subsubsection{Linear Memory and Addressing Modes}
...
Through the JVM, the compiled Java program preserves its platform independence whilst using the benefits introduced by a VM~\cite[Chapter~1.2]{Lindholm2014-jb}. %TODO: is chapter the right noun (use section?)
